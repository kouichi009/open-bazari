//
//  DraftToNewPostViewController.swift
//  GadgetMarket
//
...
class DraftToNewPostViewController: UIViewController {
...
extension DraftToNewPostViewController: UITableViewDataSource, UITableViewDelegate {
...
extension DraftToNewPostViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension DraftToNewPostViewController: UICollectionViewDataSource {
...
extension DraftToNewPostViewController: CategoryTableViewControllerDelegate, ProductStatusTableViewControllerDelegate, BrandTableViewControllerDelegate, ShipInfoWithSubTitleTableViewControllerDelegate, ShipInfoTableViewControllerDelegate, ExplainProductViewControllerDelegate, CheckTableViewCellDelegate, TitleInputTableViewCellDelegate, PriceInputTableViewCellDelegate {
    ...
    func uploadDraft() {
        ...
        HelperService.draftPostDataToServer(tupleImages: tupleImages, ratios: ratios, imageCount: tupleImages.count, title: titleText, caption: explainText, category1: Config.CAMERA, category2: category, status: status, shipPayer: shipPerson, shipWay: shipWay, shipDate: shipDate, shipFrom: shipFromPrefecture, price: inputPrice, brand: brandStr, onSuccess: {
            ...
            Api.DraftPost.REF_MYDRAFTPOST.child(self.currentUid).child(self.post.id!).removeValue()
            Api.DraftPost.REF_DRAFTPOSTS.child(self.post.id!).removeValue()
    ...
    func newPost() {
        ...
        if tupleImages.count == 0 {
            ...
            if inputPrice! < Config.minimumPrice {
                ProgressHUD.showError("商品価格は、\(Functions.formatPrice(price: Config.minimumPrice))円以上で設定してください。")
                self.tableView.reloadData()
                return;
...
extension DraftToNewPostViewController: PostPhotoCollectionViewCellDelegate {
